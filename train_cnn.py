...
#     content_idx = content[0][j + 1]
#     print(aozora_contents[content_idx])
#
...
i = 0
for auth_number in auth_target:
    auth_number = load_integ(auth_number, auth_dir = '{}{}/'.format(aozora_dir, auth_number))
    auth_number = np.array(auth_number.ix[1:,1:])
    auth_id=np.array([np.zeros(len(auth_number))+i]).T
    auth_number = np.array([auth_number[:, 1]]).T
    auth_number=np.hstack((auth_number,auth_id))
    if i==0:
        data_all=auth_number
    elif i==1 :
        break
    else:
        data_all=np.concatenate([data_all , auth_number],axis=0)
...
# for i in range(len(auth_target)-1):
#      data_integ = np.vstack(auth_target[i] , auth_target[i+1])
...
def train(inputs, targets, batch_size=100, epoch_count=100, max_length=200, model_filepath=aozora_dir + "model.h5", learning_rate=0.001):
    ...
    target = os.path.join('/tmp', 'weights2.*.hdf5')
    ...
    csv_logger_file = '/tmp/cnn_training.log'
